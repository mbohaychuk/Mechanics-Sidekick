...
- **v1 finalize analyzes the full authoritative recording, not per-step seq windows.** DC6 keeps `seq_start`/`seq_end` on each `StepState` (the per-step capture boundaries), but the v1 report builder runs the anomaly rules over the whole recorded session for simplicity and completeness (it catches anomalies anywhere in the run). Per-step, seq-range-scoped analysis is reserved for a future iteration; the recorded rows are still read authoritatively (no latest-wins drops), which is the half of DC6 that matters for correctness.
