...
SYSTEM_PROMPT = (
    ...
    "find it rather than supplying a value from general knowledge or by analogy to a similar part or "
    "a different engine. Each excerpt is labeled with the engine/section it came from (e.g. "
    "'ENGINE - 5.0L 32V TI-VCT | ...'); prefer excerpts whose engine matches this vehicle. A numeric "
    "spec from a different engine does not transfer — do not give it as this vehicle's. A general "
    "repair procedure documented under another engine's section may still apply; you may share it, "
    "but state which section it came from so the user can verify. When a "
